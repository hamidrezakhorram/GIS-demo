...
class Points(models.Model):
    ...
    audio = models.FileField(
        upload_to="media/locations/audio", blank=True, null=True
    )
    video = models.FileField(
        upload_to="media/locations/video", blank=True, null=True
    )
    geom = gis_models.PointField()
    point_type = models.CharField(
        max_length=3, choices=StatusChoices.choices, default=StatusChoices.NEW
    )
